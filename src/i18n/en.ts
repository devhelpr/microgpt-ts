...
export const en: LocaleStrings = {
  ...
  explainerBodies: {
    ...
    forward: `This is one full pass through the network. We take the token IDs and their positions, turn them into vectors (embeddings), and add them. Then we run that through: a normalization step (<strong>RMSNorm</strong>), <strong>attention</strong> (so each position can look at the others), a small <strong>MLP</strong> block, and finally a head that outputs one score per possible next character; those scores are the <strong>logits</strong>. No learning happens here; we're just computing the model's current prediction.<br/><br/><strong>What is the MLP?</strong> MLP stands for <em>Multi-Layer Perceptron</em>. It's a small feed-forward network: two linear layers with a ReLU activation in between. It processes each position independently to further transform the representations after attention has mixed information across the context.`,
  ...
  dialogs: {
    ...
    transformer: {
      title: 'Transformer structure and flow',
